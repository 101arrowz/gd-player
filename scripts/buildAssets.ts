...
import { oidMap, btnMap, modeMap } from './maps';
...
const parseSSV = (v: string): SSV => JSON.parse(v.replace(/{/g, '[').replace(/}/g, ']'));

for (const dir of ['.', 'fonts', 'spritesheets', 'backgrounds', 'groundtiles', 'sliders', 'explosions']) {
  const td = to(dir);
...
for (const f of readdirSync(rsd)) {
  ...
  if (en == '.fnt') {
    ...
    if (nm.endsWith('-uhd')) {
      ...
      splt[0] = splt[0].replace(/face="(.*)/, 'face="' + (fntMap[bn] || ('f' + +bn)) + '"') + ' size=24';
    ...
    if (nm.endsWith('-uhd')) {
      ...
      let c: string;
      if (bn.startsWith('PlayerExplosion')) c = to('explosions', (+bn.slice(16, 18)) + '');
      else if (bn.startsWith('GJ_GameSheet')) c = to('spritesheets', bn[12] == 'G' ? 'glow' : ((+bn.slice(12)) || 1) + '');
      ...
      if (c) {
        ...
        for (const k in pl.frames) {
          ...
          const [[x, y], [w, h]] = parseSSV(fr.textureRect) as [[number, number], [number, number]];
          const s = parseSSV(fr.spriteSourceSize) as [number, number];
          const frame: PixiSpriteSheetFrame = { frame: { x, y, w, h } };
          if (s[0] != x || s[1] != y) frame.spriteSourceSize = { x: s[0], y: s[1] };
          if (fr.textureRotated) frame.rotated = true;
          let key = k.slice(0, -4), add = '';
          ...
          else if (btnMap[key]) out.frames['btn' + btnMap[key] + '.png'] = frame;
          else if (key.endsWith('Btn_001')) {
            const tk = key.slice(0, -7).replace(/^(((GJ)|(gj))_)/, '');
            // Not perfect but I can't make a mapping for every single object just yet. The idea is to come back to it when necessary.
            out.frames['btn' + tk.replace(/(^.)|(_[a-zA-Z])/g, c => (c[1] || c[0]).toUpperCase()) + '.png'] = frame;
          }
          else if (key.startsWith('playerDash2_boom2')) out.frames['dashBoom' + +key.slice(-3) + '.png'] = frame;
          else if (key.startsWith('playerDash2')) out.frames['dash' + +key.slice(-3) + (key.slice(-10, -3) == 'outline' ? '.glow' : '') + '.png'] = frame;
          else if (key.startsWith('spiderDash')) out.frames['spiderDash' + +key.slice(-3) + '.png'] = frame;
          else if (key.startsWith('playerExplosion')) out.frames['explosion' + +key.slice(16, 18) + '.' + +key.slice(-3) + '.png'] = frame;
          else if (/^((bird)|(dart)|(player((_ball)?))|(robot)|(ship)|(spider))/.test(key)) {
            const ki = key.indexOf('_', key.startsWith('player_ball') ? 11 : 0), nm = modeMap[key.slice(0, ki)], tk = key.slice(ki + 1).split('_').slice(0, -1);
            let i = /^((robot)|(spider))/.test(key);
            out.frames[nm + +tk[0] + ((i && tk[1]) ? '.' + +tk[1] : '') + (tk[1 + (i as unknown as number)] ? '.' + +tk[1 + (i as unknown as number)] : '') + '.png'] = frame;
          }
          else if (key.startsWith('portal')) {
            if (key.indexOf('back') != -1) {
              const id = oidMap[key.replace('back', 'front')];
              for (const k of id) out.frames[k + '.under.png'] = frame;
              multiObjs = multiObjs.concat(id);
            } else {
              const id = oidMap[key.replace('_001', '_front_001')];
              for (const k of id) out.frames[k + '.shine.png'] = frame;
            }
          }
        }
        // animations
        if (bn == 'GameSheet') {}
        // if (bn.startsWith('PlayerExplosion')) out.animations = 
    ...
    if (nm.endsWith('-uhd')) {
      const bn = nm.slice(0, -4);
      if (bn.startsWith('game_bg')) copyFileSync(from(f), to('backgrounds', +bn.slice(8, 10) + '.png'));
      else if (bn.startsWith('groundSquare')) copyFileSync(from(f), to('groundtiles', +bn.slice(13, 15) + bn.slice(15, bn.lastIndexOf('_')).replace('_', '.') + '.png'));
      else if (bn.startsWith('slider')) {
        const s = bn.slice(6), p = s.length - 3;
        if (s.startsWith('Bar')) copyFileSync(from(f), to('sliders', p + '.png'));
        else copyFileSync(from(f), to('sliders', p + '.groove.png'));
      }
    }
  }
}

writeFileSync(to('multiObjs.json'), JSON.stringify(multiObjs).replace(/,/g, ', '));